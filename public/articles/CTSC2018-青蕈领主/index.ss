($article
  ((tag Math Poly) (title . "CTSC2018-青蕈领主") (date . "") (mathjax . #t))
  ((h2 (id . "description")) "Description")
  (p "定义一个排列中的一段区间是连续的，\n" "当且仅当这个排列中的最大值与最小值的差为这段区间的长度  $-1$。")
  (p "对于一个长度为 $n$ 的排列，\n" "给定对于 $i \\in [1,n]$，以 $i$ 这个位置结尾的最长连续区间的长度 $L_i$。")
  (p "询问满足条件的排列的个数（不存在输出" (code "-1") "）。")
  ((h2 (id . "tutorial")) "Tutorial")
  ((h3 (id . "part-1-%E6%97%A0%E8%A7%A3%E7%9A%84%E5%88%A4%E5%AE%9A"))
   "Part 1 无解的判定")
  (p
   "不难发现，如果存在 $l_1,r_1,l_2,r_2(r_2\\geqslant l_1)$，\n"
   "使得 $[l_1, r_1]$ 和 $[l_2,r_2]$ 都是合法区间，那么 $[l1, r2]$ 也必然为合法区间。")
  (p "所以存在解等价于 所有区间两两要么相互包含要么不相交，\n" "并且 $L_n = n$。")
  ((h3
    (id
     .
     "part-2-l-1-1-cdots-1-n-%E6%97%B6%E7%9A%84%E5%AD%90%E9%97%AE%E9%A2%98"))
   "Part 2 $L = {1,1,\\cdots,1,n}$ 时的子问题")
  (p "令 $f_n$\u200B 表示满足 $L_i=1\\space (i\\in [1,n])$ 的长度为 $n+1$ 的排列数量。")
  (p "使用类似与错位排列的方法求解。")
  (p "考虑将长度为 $n$ 的排列中每个元素 $+1$，再插入一个 $1$ 来得到一个长度为 $n+1$ 的排列")
  (p "分两种情况：")
  (ul
   (li
    (p
  ...
  ((h3 (id . "part-3-%E5%A6%82%E4%BD%95%E5%BF%AB%E9%80%9F%E6%B1%82-f"))
   "Part 3 如何快速求 $f$")
  (p "发现 $n \\leqslant 10000$，所以我们并不能暴力递推这个数列。")
  (p "这个递推形式看上去很像分治FFT，但这里与分治FFT模板有所不同。")
  (p
   "按照一般的分治FFT思路，我们在求 $[l,m]$ 区间对 $[m+1,r]$ 的贡献的时候，\n"
   "是把 $[l,m]$ 和 $[0,r-l]$ 这个区间卷起来，")
  (p "但在这道题中 $[0,r-l]$ 区间中 $f$ 的值可能仍未确定。")
  (p
   "解决办法是把 $f(k)\\times f(n-k+1)$ 的贡献算在 $\\max(k, n-k+1)$ 上，\n"
   "而不是一般的 $k$ 上。")
  (p "这样可以保证卷的区间已经求出。")
  (p "并且我们注意到 $(n-k)f(k)f(n-k+1) + (k-1)f(n-k+1)f(k) = (n-1)f(k)f(n-k+1)$")
  (p "所以只要给系数乘上 $n-1$ 即可。")
  (p "这部分的 code：")
  (pre
   ((code (class . "lang-cpp"))
    ...
    " cdq(mid+1, r);\n"
    "}\n"))
  ((h3 (id . "part-4-%E6%89%A9%E5%B1%95%E5%88%B0%E5%8E%9F%E9%97%AE%E9%A2%98"))
   "Part 4 扩展到原问题")
  (p "由 Part 1 可知，原问题中的区间一定构成了一个树形结构。")
  (p "考虑在儿子答案已知的情况下求点 $u$ 的方案数。")
  (p "由于每个儿子都是一段连续区间，而这些连续区间不能合并。")
  (p "不难想到把每个儿子表示的区间缩成一个点，这样加上这个点本身，\n" "组合的方案数为 $f(\\deg u)$")
  (p "所以这个点的答案为儿子的方案书之积乘上 $f(\\deg u)$")
  (p "用一个栈维护这个过程即可")
  (p "这部分的 code：")
  (pre
   ((code (class . "lang-cpp"))
