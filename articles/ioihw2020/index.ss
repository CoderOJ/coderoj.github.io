...
```)))
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ...
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    ...
    (p
     "然后倒过来 dp。令 $\\operatorname{dp}(i,j)$ 表示长度为 $i$，使用 $j$ 种数的方案数。考虑转移，枚举序列中最小值第一次出现的位置 $k$ 和被删除的时间 $p$，" 
     "可以发现前半部分的操作不受影响，并且该位置需要等后面全部删除以后才能删除。那么就有如下转移：")
    ((script (type . "math/tex; mode=display"))
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ...
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    ...
    (p "然后直接枚举答案暴力转移即可。直接枚举复杂度是 $O(2^nn^2)$ 的，但如果 dfs 所有答案，复杂度为 $O(\\sum_{i=0}^{n-1} 2^i(n-i)) = O(2^n)$。")))
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    ...
    (p "一条边被正向经过任意次会产生 1 的贡献，反向经过任意次产生 1 的贡献。（一条边贡献最多为 2）")
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    ...
    (p "$T\\leq 20，N \\leq 50$."))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    (p n 和 T 不大，只要有个多项式做法就可以了。)
    (p 如果能固定下来一个根，那么这个问题会好做很多。考虑直接枚举第一次操作那个点，接到哪个点上，那么这个点以后就一定不会被移动，可以钦定它为根。)
    (p 钦定了根以后，发现一个点被移动，当且仅当它在两棵树中的父亲不同。因此答案（如果有）是确定的，只需判断是否存在一个合法操作序列即可。)
    (p 一个点当前可以被移动的条件是它在树 A 中是叶子，并且它在树 B 中的父亲不需要被移动或已经移动过了。每次任意选能移的移即可。)))
  (h3 AGC028C)
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    (p
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ...
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    ...
    (p "因此可以通过霍尔定理判断合法性。对于构造方案，只要不断尝试加边判断合法性即可。")
    (p "时间复杂度可以接受。")))
  ((h3 (id . "cf613e")) "CF613E")
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
...
给定一个字符串 $S$，要求构造字符串序列 $s_1,s_2,\ldots,s_k$，满足任意 $s_i$ 都是 $S$ 的子串，且任意 $i\in[2,n]$，都有 $s_{i-1}$ 在 $s_i$ 中出现了至少 $2$ 次（可以有重叠部分，只要起始、结尾位置不同即可）。```)
  ...
  ((div (style . "display: flex; flex-wrap: wrap; align-items: flex-start"))
   ((div (style . "flex: 1; min-width: 400px") (class . "box")) 
      (p 给定 $m$ 个布尔变量 `$x_1, x_2, \ldots, x_m$` ，以及 $n$ 个形如 $x_i$ 或者 `$x_i \operatorname{or}  x_j$` 的布尔表达式，其中规定 `$x_{-i}=\lnot x_i$` 。
         并且，保证 $x_i$ 和 "$x_{-i}$" 在所有的布尔表达式中一共只会出现最多两次。)
      (p 请你求出，在一共 $2^m$ 种布尔变量取值的情况下，有多少种情况，使得所有布尔表达式的值的异或为 `$\rm True$`。)
      (p 由于结果可能过大，请输出答案模 $10^9+7$ 的结果。)
      (p `$1 \leq n,m \leq 10^5$` 。))
   ((div (style . "flex: 1; min-width: 400px") (class . "box"))
    (p 考虑把一个布尔表达式看作边，每个变量看作点，那么构成了一个点度最多为 2 的无向图。
       显然这样的图只可能由环或链构成。)
    (p 对于链的情况，显然有一个线性的 dp，求出对于这个连通块异或和为 0 或 1 时有多少种方案。
       对于环的情况，预先钦定一个点，断环为链以后也是同样处理。)))
