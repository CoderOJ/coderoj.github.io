($article
  ((tag Graph Tree) (title . "支配树学习笔记"))
  ((h2 (id . "%E5%AE%9A%E4%B9%89")) "定义")
  (p
   "对于一个有向图 $G$ 和起点 $s$，定义 $u$ 支配 $v$ 当且仅当所有从 $s$ 到 $v$ 的路径都经过点 $u$。支配树即所有支配关系构成的树。")
  (p "不失一般性，在后文中都假设起点 $s$ 能到达图中的所有点。")
  ((h2 (id . "dag-%E4%B8%8A%E7%9A%84%E5%81%9A%E6%B3%95")) "DAG 上的做法")
  (p "考虑按照拓扑序构建支配树。一开始树中仅有 $s$ 结点。")
  (p
   "按照拓扑序，当处理点 $u$ 的时候，所有有边连向 $u$ 的点在支配树上的位置都已经确定。显然点 $u$ 在支配树上的父亲是那些点在支配树上的 $\\operatorname{lca}$，直接倍增处理即可。")
  ((h2
    (id . "%E4%B8%80%E8%88%AC%E5%9B%BE%E4%B8%8A%E7%9A%84%E5%81%9A%E6%B3%95"))
   "一般图上的做法")
  (p
   "没有特殊说明，后文中结点的大小关系均指 $\\operatorname{dfn}$ 的大小关系，一个结点的父亲/祖先均指这个点在 dfs 树上的父亲/祖先。")
  ((h3 (id . "%E5%AE%9A%E4%B9%89")) "定义")
  (p
   "如果对与点 $u,v$ 满足存在路径 $u\\to v_1 \\to \\cdots \\to v_k \\to v$ 且 $\\forall i \\in [1,k], u_i > v$ ，那么我们称 $u$ 是 $v$ 的"
   (strong "半支配点")
   "。")
  (p "显然一个点的半支配点有多个，定义 $u$ 的" (strong "最小半支配点") "为 $\\operatorname{semi}(u)$")
  ((h3 (id . "%E6%80%A7%E8%B4%A8")) "性质")
  ((h4
    (id
     .
     "1-operatorname-semi-u-%E4%B8%80%E5%AE%9A%E6%98%AF-u-%E7%9A%84%E7%A5%96%E5%85%88"))
   "1. $\\operatorname{semi}(u)$ 一定是 $u$ 的祖先")
  (p "证明：")
  (p "显然点 $u$ 的自己满足存在路径的条件，因此 $\\operatorname{semi}(u) \\leq u$")
  (p
  ...
  ((h4
    (id
     .
     "2-%E5%88%A0%E9%99%A4%E5%8E%9F%E5%9B%BE%E9%9D%9E%E6%A0%91%E8%BE%B9%EF%BC%8C%E5%8A%A0%E5%85%A5%E5%BD%A2%E5%A6%82-operatorname-semi-u-u-%E7%9A%84%E8%BE%B9%EF%BC%8C%E6%94%AF%E9%85%8D%E5%85%B3%E7%B3%BB%E4%B8%8D%E5%8F%98"))
   "2. 删除原图非树边，加入形如 $(\\operatorname{semi}(u), u)$ 的边，支配关系不变")
  (p "证明：")
  (p
   "形如 $(\\operatorname{semi}(u), v)$ 的边在原图中对应一条路径，所以新图中不会引入新的路径，因此原图中的支配关系在新图中必然存在。")
  (p "然后我们只需要证明对于新图中的支配关系 $(x,u)$ 一定在原图中存在。")
  (p
  ...
  ((h3 (id . "%E5%81%9A%E6%B3%95")) "做法")
  (p
   "先求出所有点的最小半支配点，然后删除非树边，加入形如 $(\\operatorname{sami}(u), u)$ 的边，此时我们得到了一个与原图支配等价的 DAG，直接套用 DAG 上的做法即可（有更优秀的做法但我不会）。")
  (p "接下来问题转化为如何求出所有的最小半支配点")
  (p "考虑一条边 $u \\to v$，根据大小关系分类讨论")
  (ul
   (li "$u < v$，那么 $u$ 是 $v$ 的半支配点")
   (li "$u > v$，那么对于满足 $p > v$ 的 $u$ 的祖先 $p$，$p$ 的半支配点都是 $u$ 的半支配点"))
  (p "显然这覆盖到了所有半支配点的情况。")
  (p
